...
def filter_movies(spark):
    try:
        movies = spark.read.json('./scripts/sg_data/movies.json',multiLine=True)
        ...
        movies.select('id').toPandas().to_csv('./scripts/sg_data/end_filtered_movies_id.csv',header=True,index=False)
        movies.select('id','title','tagline','overview','poster_path','original_language','production_companies',
                      'genres','release_date','runtime','popularity','vote_average','vote_count').write.json('./scripts/sg_data/movies_end_filtered')
    except Exception as e:
        print('Error', e)
    
def transform_keywords(spark):
    try:
        keywords = spark.read.json('./scripts/sg_data/keywords.json',multiLine=True)
        ...
        keyword.select('id','name').distinct().write.json("./scripts/data_db/keyword",mode='overwrite')
        keyword.select('movie_id','id').withColumnRenamed('id','keyword_id').write.json("./scripts/data_db/keywordMovie",mode='overwrite')
        print("KEYWORD")
    except Exception as e:
        print('Error', e)
...
def transform_credits(spark):
    try:
        credits = spark.read.json('./scripts/sg_data/credits.json',multiLine=True)
        ...
        cast.select('id','movie_id','character').withColumnRenamed('id','person_id').write.json("./scripts/data_db/cast",mode='overwrite')
        crew.select('id','movie_id','job').withColumnRenamed('id','person_id').write.json("./scripts/data_db/crew",mode='overwrite')
        person = crew.union(cast)
        person.dropDuplicates(['id']).select('id','name','known_for_department','popularity').write.json('./scripts/data_db/person',mode='overwrite')
        print('CREDITS')
    except Exception as e:
        print('Error', e)
...
def transform_company(spark):
    try:
        movies = spark.read.json('./scripts/sg_data/movies_end_filtered',schema = movie_schema)
        ...
        company.select('id','movie_id').withColumnRenamed('id','company_id').write.json('./scripts/data_db/companyMovie',mode='overwrite')
        company.select('id','name').dropDuplicates(['id']).write.json('./scripts/data_db/company',mode='overwrite')
        print("COMPANY")
    except Exception as e:
        print('Error', e)
...
def transform_genre(spark):
    try:
        ...
        genre.select('id','movie_id').withColumnRenamed('id','genre_id').write.json('./scripts/data_db/genreMovie',mode='overwrite')
        genre.select('id','name').dropDuplicates(['id']).write.json('./scripts/data_db/genre',mode='overwrite')
        print("GENRES")
    except Exception as e:
        print('Error', e)
...
def transform_movies(spark):
    try:
        movies = spark.read.json('./scripts/sg_data/movies_end_filtered')
        movies.select('id','title','tagline','overview','poster_path','original_language','release_date','runtime',
                      'popularity','vote_average','vote_count').write.json('./scripts/data_db/movie',mode='overwrite')
        print("MOVIES")
    except Exception as e:
        print('Error', e)