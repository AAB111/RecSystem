...
class ColumnCombiner:
    ...
    def tto(movie,transform_column='combined_column'):
        movie = movie.withColumn(transform_column, F.concat(movie["title"], F.lit(" "), movie["tagline"], F.lit(" "), movie["overview"]))
        return movie
    
    @staticmethod
    def combination_tto_genres_keywords_actors_directors(movie,genre_movie,genre,keyword_movie,keyword,cast,crew,person,transform_column='combined_column'):
    ...
    @staticmethod
    def combination_genres_keywords(movie,genre_movie,genre,keyword_movie,keyword,transform_column='combined_column'):
        joined_df = movie.join(genre_movie, movie["id"] == genre_movie["movie_id"], "left")
    ...
    @staticmethod
    def combination_tto_keywords_actors_directors(movie,keyword_movie,keyword,crew,cast,person,transform_column='combined_column'):
    ...
    def combination_tto_characters_actors_directors(movie,crew,cast,person,transform_column='combined_column'):
        ...
        top_actors = top_actors.withColumn("actor_character_pairs", combine_actor_character_udf(F.col("actor_name"), F.col("character")))
        top_actors = (top_actors.
                    groupBy("movie_id").agg(
...
class MatrixSim:
    ...
    def matrix_sim_between_dfs(left_df,right_df):
        ...
        transformed_df = transformed_df.withColumn('cos_sim',cos_sim(F.col('normalized_features'),F.col(f'normalized_features_right')))
