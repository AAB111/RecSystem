...
class ContentBasedForUser(ContentBased):
    def __init__(self, base_model, movie, movie_be_watch, movie_eval, movie_neg, movie_watch, crew, cast, person,
                 sim_matrix_movies):
    ...
    def recommend_sim(self, user_id, top_n_for_movie=20):
        filtered_movie_be_watch = self.movie_be_watch.filter(F.col("user_id") == user_id).select('movie_id', 'user_id',
                                                                                                 'datetime_added')
        ...
        high_rated_movies = filtered_movie_eval.filter(F.col('rating') >= avg_rating).select('movie_id', 'user_id',
                                                                                             'datetime_added')
        filtered_movie_neg = self.movie_neg.filter(F.col("user_id") == user_id).select('movie_id', 'user_id',
                                                                                       'datetime_added')
        filtered_movie_watch = self.movie_watch.filter(F.col("user_id") == user_id).select('movie_id', 'user_id',
                                                                                           'datetime_added')
        ...
        all_user_movies_filtered = all_user_movies.filter(
            F.col('datetime_added') >= F.current_date() - half_year_ago).select('movie_id')
        movie_ids = [row['movie_id'] for row in all_user_movies_filtered.collect()]
        similar_movies = self.sim_matrix_movies.filter(F.col('id').isin(movie_ids)).orderBy(
            F.col('cos_sim').desc()).limit(len(movie_ids) * top_n_for_movie + len(movie_ids))
        num_rows_to_drop = len(movie_ids)
        similar_movies = similar_movies.exceptAll(similar_movies.limit(num_rows_to_drop))

        filtered_movie_neg = self.movie.filter(
        ...
        filtered_movie_neg = ColumnCombiner.combination_tto_characters_actors_directors(
            filtered_movie_neg.select('id', 'title', 'tagline', 'overview'), self.crew, self.cast, self.person,
            'combined_column')
...
class ContentBasedAuto(ContentBasedForUser):
    def __init__(self, base_model, sim_matrix_movies, user, movie, movie_be_watch, movie_eval, movie_neg, movie_watch,
                 crew, cast, person):
        super().__init__(base_model, movie, movie_be_watch, movie_eval, movie_neg, movie_watch, crew, cast, person,
                         sim_matrix_movies)
        self.user = user
