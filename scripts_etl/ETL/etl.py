...
async def async_get_movie_id_by_title_date(handler,file_path_load,file_path_save):
    try:
        reader = ReaderWriter()
        movies_id = await asyncio.gather(*[handler(dict['name'],dict['date']) async for dict in reader.async_generator_read_file_csv(file_path_load)])
...
async def main():
    ...
    try:
        ...
        load_data_to_db("../data_db/movie", "Movie",spark,schema=schema_movie)
        load_data_to_db("../data_db/genre", "Genre",spark)
        load_data_to_db("../data_db/company", "Company",spark)
        load_data_to_db("../data_db/person", "Person",spark)
        load_data_to_db("../data_db/keyword", "Keyword",spark)
        load_data_to_db("../data_db/cast", "Cast",spark)
        load_data_to_db("../data_db/companyMovie", "CompanyMovie",spark)
